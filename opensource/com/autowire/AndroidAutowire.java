...
public class AndroidAutowire {
	...
	/**
	 * Perform the wiring of the Android View using the {@link AView} annotation.
	 * <br /><br />
	 ...
	 * {@link ALayout} annotation, or a valid layout id will not be returned.  This will work with Activity,
	...
	public static int getLayoutResourceByAnnotation(Object thisClass, Context thisActivity, Class<?> baseClass) {
		ALayout layoutAnnotation = thisClass.getClass().getAnnotation(ALayout.class);
		Class<?> clazz = thisClass.getClass();
		while(layoutAnnotation == null && baseClass.isAssignableFrom(clazz.getSuperclass())){
			clazz = clazz.getSuperclass();
			layoutAnnotation = clazz.getAnnotation(ALayout.class);
		}
	 ...
	 * with the {@link ASaveInstance} annotation.  These will be put in the Bundle object.
	...
	public static void saveFieldsToBundle(Bundle bundle, Object thisClass, Class<?> baseClass){
		...
		while(baseClass.isAssignableFrom(clazz)){
			for(Field field : clazz.getDeclaredFields()){
				if(field.isAnnotationPresent(ASaveInstance.class)){
	 ...
	 * with the {@link ASaveInstance} annotation. Get the saved value for these fields from the Bundle, and 
	...
	/**
	 * Autowire a custom view class. Load the sub views for the custom view using the {@link AView} annotation.
	 ...
	 * Will not be thrown if required=false on the {@link AView} annotation.
	 */
	...
	private static void autowireViewsForFragment(Object thisFragment, Class<?> clazz, View contentView, Context context){
		for (Field field : clazz.getDeclaredFields()){
			...
			AView androidView = field.getAnnotation(AView.class);
